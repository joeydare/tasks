...
export const removeDollars = (amounts: string[]): number[] => {
    return amounts.map((str) => parseInt(str.replace("$", "")) || 0);
};
...
export function countShortWords(words: string[]): number {
    const lessThan4 = words.filter((word) => word.length < 4);
...
export function injectPositive(values: number[]): number[] {
    ...
    if (negIndex === -1) {
        return [...values, values.reduce((sum, currNum) => sum + currNum, 0)];
    }
    return [
        ...values.slice(0, negIndex),
        values[negIndex],
        values.slice(0, negIndex).reduce((sum, currNum) => sum + currNum, 0),
