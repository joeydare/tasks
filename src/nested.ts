...
export function sumPoints(questions: Question[]): number {
    return questions.reduce(
        (sumPoints, question) => sumPoints + question.points,
        0
    );
}
...
export function sumPublishedPoints(questions: Question[]): number {
    const publishedQ = questions.filter((question) => question.published);
...
export function toCSV(questions: Question[]): string {
    ...
    const rows = questions.map((question) => {
        const optionsNum = question.options ? question.options.length : 0;
        return `${question.id},${question.name},${optionsNum},${question.points},${question.published}`;
    });
...
export function renameQuestionById(
    ...
    return questions.map((question) =>
        question.id === targetId ? renameQuestion(question, newName) : question
    );
}
